...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((layerState, i) => {
      tl.fromTo(
        layerState.el,
  ...
  const shouldRenderHeader = !hideToggleButton;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {shouldRenderHeader && (
          <header
            className="staggered-menu-header absolute left-0 top-0 z-20 flex w-full items-center justify-between bg-transparent p-[2em] pointer-events-none"
            aria-label="Main navigation header"
          >
            <div className="sm-logo pointer-events-auto flex select-none items-center" aria-label="Logo">
              <img
                src={logoUrl || "/jp-mark.svg"}
                alt="Logo"
                ...
                >
                  <span ref={textInnerRef} className="sm-toggle-textInner flex flex-col leading-none">
                    {textLines.map((line, index) => (
                      <span className="sm-toggle-line block h-[1em] leading-none" key={index}>
                        {line}
                      </span>
                    ))}
                  </span>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute right-0 top-0 z-10 flex h-full flex-col overflow-y-auto bg-white p-[6em_2em_2em_2em] pointer-events-auto backdrop-blur-[12px]"
              ...
              {items && items.length ? (
                items.map((item, index) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={`${item.label}${index}`}>
                    <a
                      className="sm-panel-item relative inline-block pr-[1.4em] text-[4rem] font-semibold uppercase leading-none tracking-[-2px] text-black no-underline transition-[background,color] duration-150 ease-linear cursor-pointer"
                      href={item.link}
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative inline-block pr-[1.4em] text-[4rem] font-semibold uppercase leading-none tracking-[-2px] text-black no-underline transition-[background,color] duration-150 ease-linear cursor-pointer">
                    <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
                      No items
                    </span>
                  </span>
...
.sm-scope [data-position='left'] .sm-prelayer { right: auto; left: 0; }
